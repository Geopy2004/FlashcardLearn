...
class _MathQuizPageState extends State<MathQuizPage> {
  final List<Map<String, dynamic>> mathFlashcards = getFormattedQuizData();
  int _currentIndex = 0;
  int _score = 0;
    ...
    setState(() {
      if (_answerSelector.answered) {
        if (_answerSelector.selectedAnswerIndex ==
            mathFlashcards[_currentIndex]['correctIndex']) {
          _score++;
        }
      }
      _currentIndex++;
        ...
        return AlertDialog(
          ...
          content: Text(
            isCorrect
                ...
                : 'Oops! The correct answer was: ${mathFlashcards[_currentIndex]['options'][mathFlashcards[_currentIndex]['correctIndex']]}',
          ),
          actions: [
            TextButton(
              onPressed: () {
  ...
  Widget build(BuildContext context) {
    if (_currentIndex >= mathQuizData.length) {
      return Scaffold(
        appBar: AppBar(
        ...
        body: Center(
          child: Column(
            ...
            children: [
              Text(
                'Your Score: $_score/${mathFlashcards.length}',
    ...
    final currentCard = mathFlashcards[_currentIndex];
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Question ${_currentIndex + 1}/${mathFlashcards.length}',
              style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold),
            ),
            SizedBox(height: 20),
            Flashcard(
