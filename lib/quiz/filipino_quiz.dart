...
class FilipinoQuizPage extends StatefulWidget {
  @override
  _FilipinoQuizPageState createState() => _FilipinoQuizPageState();
}

class _FilipinoQuizPageState extends State<FilipinoQuizPage> {
  final List<Map<String, dynamic>> filipinoFlashcards = getFormattedQuizData();
  int _currentIndex = 0;
  int _score = 0;
  ...
  void _nextQuestion() {
    if (_currentIndex >= filipinoFlashcards.length - 1) {
      _endQuiz();
      return;
    }
    ...
    setState(() {
      if (_answerSelector.answered) {
        if (_answerSelector.selectedAnswerIndex ==
            filipinoFlashcards[_currentIndex]['correctIndex']) {
          _score++;
        }
      }
      _currentIndex++;
        ...
        return AlertDialog(
          ...
          content: Text(
            isCorrect
                ...
                : 'Oops! The correct answer was: ${filipinoFlashcards[_currentIndex]['options'][filipinoFlashcards[_currentIndex]['correctIndex']]}',
          ),
          actions: [
            TextButton(
              onPressed: () {
  ...
  Widget build(BuildContext context) {
    if (_currentIndex >= filipinoFlashcards.length) {
      return Scaffold(
        appBar: AppBar(
        ...
        body: Center(
          child: Column(
            ...
            children: [
              Text(
                'Your Score: $_score/${filipinoFlashcards.length}',
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Question ${_currentIndex + 1}/${filipinoFlashcards.length}',
              style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold),
            ),
            SizedBox(height: 20),
            Flashcard(
